...
function Portal(props: {
  ...
  useFrame((state) => {
    ...
    g.position.y = Math.sin(t * 1.4 + props.position[0]) * 0.12;

    // Walk-to-open: entering the beacon radius opens the district (spec: navigation IS the game).
    // While any district is open, disarm — otherwise closing the overlay while parked at the
    // portal would instantly reopen it. Re-arming requires flying back out of the radius.
    if (active) {
      armed.current = false;
      return;
    }
    ...
    <group position={[props.position[0], 0, props.position[1]]}>
      ...
      <Html center position={[0, 5.4, 0]} style={{ pointerEvents: 'none' }} zIndexRange={[10, 0]}>
        ...
        >
          ...
          {hovered && <div style={{ color: '#8b98b3', fontSize: 9 }}>{props.tagline}</div>}
